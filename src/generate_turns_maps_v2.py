...
USAGE = """
        Generate shapefiles for turn restrictions files (.pen files). 
        To run the script, type and enter: 
        python generate_turns_maps.py [--geojson] [--map map_out_dir] shapefile_in_dir turnfile_dir shapefile_out_dir" in commandline tool
        if --geojson option is specified, the output sptialwill be geojson file. Default is shapefile
...
def find_link_in_geodataframe(geodataframe ,start_id, end_id, start_field ="A", end_field="B" ):
    ...
    # try fails, not corresponding record
        ...
        return link_record
#%%
def linestring_adjust(line1, line2):
    # Used to combine two lineString objects, whose sequence are: from line1 --> line2
       
    l1_head, *_ , l1_tail = list(line1.coords)
    l2_head, *_ , l2_tail = list(line2.coords)
    if l2_head == l1_tail:
        return LineString([*line1.coords[:], *line2.coords[1:]])
    else:
        return None
...
def offset_turn(turn_gdf):
    def random_distance():
        return np.random.choice([0, 10, 20, 30])
...
def generate_maps(base_network, overlaid_layers, output_dir, map_name, title):
    ...
    ax_map.scatter(x_ends, y_ends, s= 10, color = "orange")
...
if __name__ == "__main__":
    ...
    map_dir = "TURN_MAPS"
    turn_file = "turnspm.pen"
    network_shp = "/Users/yimingcai/Documents/SFCTA_internship/generate_turns_maps/working_folder/FREEFLOW.shp"
    network_outshp_dir = "OUTPUT_SHP"
    if not os.path.isdir(network_outshp_dir):
        os.mkdir(network_outshp_dir)
    network_outshp_name = "PM_SHP.shp"
    network_prjoutshp_name = network_outshp_name.split(".")[0] + "_PRJ.shp" 
    network_outgjson_name = "PM.geojson"
    ...
    turns_gdf = gpd.GeoDataFrame(pd.concat(data_store))
    #set a random offset 
    turns_gdf = offset_turn(turns_gdf)
    turns_gdf.to_file(network_outshp_file)
    #generate maps 
    generate_maps(network_gdf, turns_gdf, abs_map_dir ,"PM" ,"PM TURN RESTRICTION")
    subprocess.call(["ogr2ogr -a_srs EPSG:102643 {0} {1}".format(network_prjoutshp_file, 
                                                     network_outshp_file )],
                                                shell = True)
    crs = {"init":"epsg:4326"}
    try:
        gdf = gpd.read_file(network_prjoutshp_file)
        print (gdf.crs)
        #project to wgs 84
        gdf = gdf.to_crs(crs)
    except Exception as e:
